...
def post_share(request, post_id):
    ...
    # Different behaviours from the same view
    if request.method == 'POST':
        # Make the email sending with the filled user email data
        ...
        # Display an empty form for the user on the view
        form = EmailPostForm()
...
def post_comment(request, post_id):
    post = get_object_or_404(Post, id=post_id, status=Post.Status.PUBLISHED)
    comment = None
    # create a form with the post comment data
    form = CommentForm(data=request.POST)
    if form.is_valid():
        # create the obj, but not commit it until the relationship with post is make
